...
- 默认教师账号 admin / 95279527，生产凭据存 Worker secrets；记住登录使用 HttpOnly 会话 Cookie，不存明文密码。学生账号只由老师手动创建；默认随机账号、初始密码为账号后六位。服务端按角色和学生 ID 限制权限，学生不能访问班级管理、教师课程接口、备份或他人记录。
...
- 每次操作服务端判定并持久化；请求 ID 防重复、版本号防并发覆盖。跨刷新恢复当前阶段/轮次/题号/错误，完成两阶段才原子更新个人记忆间隔。课堂词表改变后旧练习失效，个人历史保留；删除课/班/学生不删除素材或记忆历史。
